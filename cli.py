...
def cmd_preview(args: argparse.Namespace) -> None:
    run_preview(book_root(args.book))


def cmd_deep(args: argparse.Namespace) -> None:
    root = book_root(args.book)
...
def cmd_approve(args: argparse.Namespace) -> None:
    approve_chapter(book_root(args.book), args.chapter)


def cmd_reset(args: argparse.Namespace) -> None:
    reset_chapter(book_root(args.book), args.chapter)


def cmd_status(args: argparse.Namespace) -> None:
    hitl_status(book_root(args.book))


def cmd_synthesis(args: argparse.Namespace) -> None:
    run_synthesis(book_root(args.book))


def cmd_index(args: argparse.Namespace) -> None:
    p = build_concept_index(book_root(args.book))
    print(f"Concept index → {p}")


def cmd_compile(args: argparse.Namespace) -> None:
    compile_skill(book_root(args.book), install=args.install)
...
def cmd_all_inspired(args: argparse.Namespace) -> None:
    ...
    # golden sample
    ch01 = root / "insight" / "chapters" / "ch01.md"
...
def main() -> None:
    ...
    sub.add_parser("migrate", help="Migrate 人人都是产品经理 → insight/").set_defaults(func=cmd_migrate)
